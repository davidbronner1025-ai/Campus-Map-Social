...
export default function NotificationBell() {
  ...
  return (
    <div className="relative">
      <button onClick={handleOpen} aria-label="התראות" className="p-2 rounded-xl hover:bg-secondary transition-colors relative">
            ...
            >
              <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                ...
                <div className="flex items-center gap-1">
                  {unreadCount > 0 && (
                    <button onClick={handleMarkAllRead} aria-label="סמן הכל כנקרא"
                      className="p-1.5 rounded-lg hover:bg-secondary transition-colors" title="Mark all read">
                      <CheckCheck className="w-3.5 h-3.5 text-muted-foreground" />
                    </button>
                  )}
                  <button onClick={() => setOpen(false)} aria-label="סגור" className="p-1.5 rounded-lg hover:bg-secondary transition-colors">
