...
const BANNER_COLORS = [
  "#0f172a", "#1e293b", "#1a2233", "#172033",
  "#0c2340", "#1a1a2e", "#16213e", "#0a1628",
  "#1c1c2e", "#200a0a", "#0d2e1a", "#1a0d2e",
];
...
export default function ProfilePage() {
  ...
  const displayAvatar = avatarUrl?.startsWith("emoji:") ? avatarUrl.slice(6) : null;

  return (
    <div className="flex-1 flex flex-col overflow-y-auto">
      {/* Banner */}
      <div className="relative h-40 flex-shrink-0" style={{ background: bannerColor }}>
        <button onClick={() => navigate("/")} className="absolute top-4 left-4 p-2 rounded-full bg-black/30 text-white backdrop-blur-sm">
      ...
      <div className="px-5 pt-14 pb-8 space-y-5">
        ...
        {/* Title */}
        <div>
          <label className="block text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1.5">Title / Status</label>
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Computer Science • 3rd year"
            className="w-full px-4 py-3 rounded-xl bg-card border border-border text-foreground focus:outline-none focus:border-primary transition-colors text-sm" />
        </div>

        {/* Avatar URL or emoji */}
        <div>
          <label className="block text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1.5">Avatar URL (or pick emoji ↑)</label>
          <input value={avatarUrl?.startsWith("emoji:") ? "" : avatarUrl} onChange={e => setAvatarUrl(e.target.value)} placeholder="https://..."
            className="w-full px-4 py-3 rounded-xl bg-card border border-border text-foreground focus:outline-none focus:border-primary transition-colors text-sm" />
        </div>

        {/* Banner color */}
        <div>
          <label className="block text-xs font-medium text-muted-foreground uppercase tracking-wider mb-2">Banner Color</label>
        ...
        <p className="text-xs text-muted-foreground text-center">{user?.phone}</p>
      </div>

      {/* Emoji picker overlay */}
      {emojiPickerOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end" onClick={() => setEmojiPickerOpen(false)}>
          <div className="w-full bg-card rounded-t-3xl p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-foreground mb-4">Choose Avatar</h3>
