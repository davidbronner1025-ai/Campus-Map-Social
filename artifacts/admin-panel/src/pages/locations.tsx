              ...
              <div key={day} className="bg-card border border-border rounded-xl p-3">
                ...
                {entries.map(e=>(
                  <div key={e.id} className="flex items-center justify-between py-1.5 border-t border-border first:border-0">
                    ...
                    <button onClick={()=>deleteSch.mutate({scheduleId:e.id},{onSuccess:()=>void reS()})}><X className="w-3 h-3 text-muted-foreground hover:text-destructive"/></button>
                  </div>
                ))}
              </div>
            );
      ...
      {games.map(g=>(
        <div key={g.id} className="bg-card border border-border rounded-xl p-3">
          <div className="flex justify-between items-start mb-2">
            ...
            <button onClick={()=>deleteGame.mutate({gameId:g.id},{onSuccess:()=>void refetch()})} className="p-1 text-muted-foreground hover:text-destructive"><Trash2 className="w-4 h-4"/></button>
          </div>
