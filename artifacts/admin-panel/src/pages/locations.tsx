...
function BuildingPanel({ locationId }: { locationId:number }) {
  ...
  const {data:anns=[],refetch:refetchAnn}=useGetAnnouncements(locationId);
  const {data:scheds=[],refetch:refetchSch}=useGetSchedules(locationId);
  ...
  const reA = () => refetchAnn();
  const reS = () => refetchSch();
...
function SportsPanel({ locationId }: { locationId:number }) {
  const {data:games=[],refetch:reG}=useGetGames(locationId);
  ...
  const submitGame=(d:any)=>createGame.mutate({locationId,data:{...d,scheduledAt:new Date(d.scheduledAt).toISOString()}},{onSuccess:()=>{gForm.reset();reG();}});
  const submitVote=(gameId:number)=>{
    if(!voteState?.name.trim()) return;
    voteGame.mutate({gameId,data:{playerName:voteState.name}},{onSuccess:()=>{setVoteState(null);reG();}});
  };
      ...
      {games.map(g=>(
        <div key={g.id} className="bg-card border border-border rounded-xl p-3">
          <div className="flex justify-between items-start mb-2">
            ...
            <button onClick={()=>deleteGame.mutate({gameId:g.id},{onSuccess:()=>reG()})} className="p-1 text-muted-foreground hover:text-destructive"><Trash2 className="w-4 h-4"/></button>
          </div>
...
export default function LocationsPage() {
  ...
  const { data: campus, isLoading: campusLoading } = useGetCampus();
