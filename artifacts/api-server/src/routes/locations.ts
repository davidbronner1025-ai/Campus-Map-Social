...
import { eq, desc, avg, and, gte, sql, inArray } from "drizzle-orm";
import {
  CreateLocationBody,
  UpdateLocationBody,
  GetLocationParams,
      ...
      if (loc.managerId) {
        try {
          const [mgr] = await db
            ...
            .from(usersTable)
            .where(eq(usersTable.id, loc.managerId));
          if (mgr)
            return { ...loc, managerName: mgr.displayName || "Assigned" };
        } catch (err) {
          console.error(
            `[locations] Failed to enrich manager for loc ${loc.id}:`,
            err,
          );
        }
      }
...
const requirePin = (req: any, res: any, next: any) => {
  ...
  if (!pin || pin.trim() !== expectedPin.trim()) {
    console.warn(
      `[locations] Unauthorized write attempt. Received PIN: [${pin}], Expected: [${expectedPin}]`,
    );
    return res
      .status(401)
      .json({ error: "Unauthorized — valid admin PIN required" });
  }
  next();
};
  ...
  try {
    ...
    if (!campusId) {
      ...
      res
        .status(400)
        .json({
          error: "Campus not configured yet. Please go to Setup page first.",
        });
      return;
    }
    ...
    const msg =
      err.name === "ZodError"
        ? "Validation failed: " + JSON.stringify(err.errors)
        : String(err);
...
router.get("/locations/:locationId", async (req, res) => {
  try {
    const { locationId } = GetLocationParams.parse({
...
router.delete("/locations/:locationId", requirePin, async (req, res) => {
  try {
    const { locationId } = DeleteLocationParams.parse({
      locationId: Number(req.params.locationId),
    });
...
router.get("/locations/:locationId/crowd", async (req, res) => {
  try {
    ...
    const rows = await db
      ...
      .where(
        and(
          gte(messagesTable.createdAt, twoHoursAgo),
...
router.get("/locations/:locationId/announcements", async (req, res) => {
  try {
    const { locationId } = GetAnnouncementsParams.parse({
      locationId: Number(req.params.locationId),
    });
    const rows = await db
      .select()
...
router.post(
  ...
  async (req, res) => {
    try {
      const { locationId } = CreateAnnouncementParams.parse({
        locationId: Number(req.params.locationId),
      });
      const body = CreateAnnouncementBody.parse(req.body);
      const created = await db
        .insert(announcementsTable)
        .values({ ...body, locationId })
        .returning();
...
router.delete(
  ...
  async (req, res) => {
    try {
      const { announcementId } = DeleteAnnouncementParams.parse({
...
router.get("/locations/:locationId/schedules", async (req, res) => {
  try {
    const { locationId } = GetSchedulesParams.parse({
      locationId: Number(req.params.locationId),
    });
    const rows = await db
      .select()
...
router.post(
  ...
  async (req, res) => {
    try {
      const { locationId } = CreateScheduleEntryParams.parse({
        locationId: Number(req.params.locationId),
      });
...
router.delete("/schedules/:scheduleId", requirePin, async (req, res) => {
  try {
    const { scheduleId } = DeleteScheduleEntryParams.parse({
...
router.get("/locations/:locationId/menus", async (req, res) => {
  try {
    const { locationId } = GetMenusParams.parse({
      locationId: Number(req.params.locationId),
    });
    ...
    const allRatings = await db
      .select()
      .from(menuRatingsTable)
      .where(inArray(menuRatingsTable.menuId, menuIds));

    const ratingsByMenuId = allRatings.reduce(
      (acc, rating) => {
        if (!acc[rating.menuId]) acc[rating.menuId] = [];
        acc[rating.menuId].push(rating);
        return acc;
      },
    ...
    const result = menus.map((menu) => {
      const ratings = ratingsByMenuId[menu.id] || [];
...
router.post("/locations/:locationId/menus", requirePin, async (req, res) => {
  try {
    const { locationId } = CreateMenuParams.parse({
      locationId: Number(req.params.locationId),
    });
...
router.post("/menus/:menuId/rate", async (req, res) => {
  try {
    const { menuId } = RateMenuParams.parse({
...
router.get("/locations/:locationId/games", async (req, res) => {
  try {
    const { locationId } = GetGamesParams.parse({
      locationId: Number(req.params.locationId),
    });
    ...
    const allVotes = await db
      .select()
      .from(gameVotesTable)
      .where(inArray(gameVotesTable.gameId, gameIds));

    const votesByGameId = allVotes.reduce(
      (acc, vote) => {
        if (!acc[vote.gameId]) acc[vote.gameId] = [];
        acc[vote.gameId].push({
          playerName: vote.playerName,
          votedAt: vote.votedAt,
        });
        return acc;
      },
      {} as Record<number, { playerName: string; votedAt: Date }[]>,
    );
...
router.post("/locations/:locationId/games", requirePin, async (req, res) => {
  try {
    const { locationId } = CreateGameParams.parse({
      locationId: Number(req.params.locationId),
    });
    ...
    const created = await db
      ...
      .values({ ...body, scheduledAt, locationId })
      .returning();
...
router.delete("/games/:gameId", requirePin, async (req, res) => {
  try {
    const { gameId } = DeleteGameParams.parse({
...
router.post("/games/:gameId/vote", async (req, res) => {
  try {
    const { gameId } = VoteForGameParams.parse({
      gameId: Number(req.params.gameId),
    });
    const body = VoteForGameBody.parse(req.body);
    await db
      .insert(gameVotesTable)
      .values({ gameId, playerName: body.playerName });
